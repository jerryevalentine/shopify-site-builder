...
# Step: Create and Populate Menus
nav_links_df = pd.read_csv('csv/navigation_links.csv')

# First, create all unique menus
unique_menus = nav_links_df['Menu Name'].unique()

for menu_name in unique_menus:
    existing_menu_id = find_navigation_menu_id_by_title(menu_name)
    
    if not existing_menu_id:
        create_navigation_menu(menu_name)

# Then, add all links
for idx, row in nav_links_df.iterrows():
    ...
    if menu_id:
        ...
        if link_type == "PAGE":
            ...
            destination_id = target_title  # e.g., "/" or external URL

        if destination_id:
            add_link_to_navigation_menu(menu_id, link_title, link_type, destination_id)
        else:
            print(f"⚠️ Could not find resource for {link_title}")
    else:
